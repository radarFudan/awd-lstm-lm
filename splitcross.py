...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def logprob(
        self, weight, bias, hiddens, splits=None, softmaxed_head_res=None, verbose=False
    ):
        ...
        if softmaxed_head_res is None:
            ...
            if self.nsplits > 1:
                head_weight = (
                    ...
                    else torch.cat([head_weight, self.tail_vectors])
                )
                head_bias = (
                    self.tail_bias
                    if head_bias is None
                    else torch.cat([head_bias, self.tail_bias])
                )
        ...
        for idx in splits:
            ...
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
                ...
                tail_res = torch.nn.functional.linear(
                    hiddens, tail_weight, bias=tail_bias
                )
    ...
    def split_on_targets(self, hiddens, targets):
        ...
        for idx in range(self.nsplits):
            ...
            split_hiddens.append(
                hiddens.masked_select(tmp_mask.unsqueeze(1).expand_as(hiddens)).view(
                    -1, hiddens.size(1)
                )
            )
    ...
    def forward(self, weight, bias, hiddens, targets, verbose=False):
        ...
        # We need to guard against empty splits as torch.cat does not like random lists
        combo = torch.cat(
            [split_hiddens[i] for i in range(self.nsplits) if len(split_hiddens[i])]
        )
        ###
        ...
        for idx in range(self.nsplits):
            ...
            # For those targets in the head (idx == 0) we only need to return their loss
            if idx == 0:
                softmaxed_head_res = softmaxed_all_head_res[
                    running_offset : running_offset + len(split_hiddens[idx])
                ]
                entropy = -torch.gather(
                    softmaxed_head_res, dim=1, index=split_targets[idx].view(-1, 1)
                )
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
            else:
                softmaxed_head_res = softmaxed_all_head_res[
                ...
                if self.verbose or verbose:
                    start, end = self.splits[idx], self.splits[idx + 1]
                    tail_weight = weight[start:end]
                    self.stats[idx].append(
                        split_hiddens[idx].size()[0] * tail_weight.size()[0]
                    )

                # Calculate the softmax for the words in the tombstone
                tail_res = self.logprob(
                    weight,
                    bias,
                    split_hiddens[idx],
                    splits=[idx],
                ...
                tail_entropy = torch.gather(
                    torch.nn.functional.log_softmax(tail_res, dim=-1),
                    dim=1,
                    index=indices,
                ).squeeze()
            ...
            total_loss = (
                ...
                else total_loss + entropy.float().sum()
            )
